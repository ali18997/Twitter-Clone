...
let clientAction clientRef controlFlag command payload =
    ...
    //printfn "%s" mmm
    clientRef <! JsonConvert.SerializeObject(clientMessage(controlFlag, command, payload))
...
let sendToServer server command payload=
    ...
    server <! JsonConvert.SerializeObject(serverMessage(command, payload))
...
let Client (ClientMailbox:Actor<_>) = 
    ...
    let rec ClientLoop() = actor {
        ...
        | :? string as m ->
            let received = JsonConvert.DeserializeObject<clientMessage>(m)
            printfn "%s" received.getCommand
            if received.getControlFlag then 
                if received.getCommand="Register" then
                    sendToServer Twitter "Register" ClientMailbox.Self.Path.Name
                elif (received.getCommand = "Send Tweet" || received.getCommand = "Subscribe" || received.getCommand = "Query") then
                    ClientToServer server received.getCommand received.getPayload
                else
                    if(not (tweetList.IsEmpty)) then 
                        let randomIndex = generateRandomNumber 0 (tweetList.Length-1)
                        ClientToServer server received.getCommand tweetList.[randomIndex]
            else
                if(received.getCommand = "Register") then
                    server <- ClientMailbox.Sender()
                    printfn "%A Registered" ClientMailbox.Self.Path.Name

                elif(received.getCommand = "MyMentions") then
                    let list:List<tweet> = downcast received.getPayload
                ...
                elif(received.getCommand = "Subscribed") then
                    let list:List<tweet> = downcast received.getPayload
                    let mutable ll = List.map (constructStringFromTweet) list
                    if ll.Length>10 then
                        ll<-take 10 ll
                ...
                elif(received.getCommand = "Hashtags") then
                    let list:List<tweet> = downcast received.getPayload
                    let mutable ll = List.map (constructStringFromTweet) list
                    if ll.Length>10 then
                        ll<-take 10 ll
                ...
                elif(received.getCommand = "Live") then
                    let liveTweet:tweet = downcast received.getPayload
        ...
        | _ ->
            Console.WriteLine ("Unexpected message at client actor")
...
let ClientCoordinator (mailbox: Actor<_>) =
    ...
    let rec loop () = actor {
        ...
        | :? string as m ->
            let received = JsonConvert.DeserializeObject<ClientCoordinatorMessage>(m)
            if received.Command="InitializeClientCoorinator" then
                noOfClients <- received.NoOfClients
            ...
            elif received.Command="RegisterAndSubscribe" then
                //printfn "xxx"
            ...
            elif received.Command="Operate" then
                let choices = [1;3]
                ...
                printfn "Crap"
        | _ ->
            Console.WriteLine ("Unexpected message at display actor")
...
cc<!JsonConvert.SerializeObject(ClientCoordinatorMessage("InitializeClientCoorinator",10))
