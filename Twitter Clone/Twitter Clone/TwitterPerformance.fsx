...
let server (serverMailbox:Actor<serverMessage>) = 
    ...
    let rec serverLoop() = actor {
        ...
        if message.getCommand = "Register" then
            ...
            //printfn "Tweet Sent"
            sendLive tweet
            serverMailbox.Sender()<!clientMessage(false, "Tweeted", null)
...
let mutable totalTweetTime = 0L
let mutable totalMentionsTime = 0L
let mutable totalSubscribeTime = 0L
let mutable totalHashTime = 0L
    //Actor


let mutable CCRef = null
        ...
        if message.getControlFlag then 
            if message.getCommand="Register" then
                ...
                if query.getTypeOf = "MyMentions" then
                    ...
                    printfn "Improper query at client"
                ClientToServer server message.getCommand message.getPayload
            else
            ...
            if(message.getCommand = "Register") then
            ...
            elif(message.getCommand = "Tweeted") then
                totalTweetTime<-totalTweetTime+tweetTimer.ElapsedMilliseconds
                totalRequests<-totalRequests+1
                totalTweets<-totalTweets+1
                tweetTimer.Stop()
                CCRef<!ClientCoordinatorMessage("TweetResponse",0)
                ...
                totalMentionsTime<-totalMentionsTime+mentionsTimer.ElapsedMilliseconds
                totalRequests<-totalRequests+1
                totalMentions<-totalMentions+1
                mentionsTimer.Stop()  
                CCRef<!ClientCoordinatorMessage("MentionResponse",0)              
                ...
                totalSubscribeTime<-totalSubscribeTime+subscribeTimer.ElapsedMilliseconds
                totalRequests<-totalRequests+1
                totalSubscribes<-totalSubscribes+1
                subscribeTimer.Stop()
                CCRef<!ClientCoordinatorMessage("SubscribedResponse",0)   
                ...
                totalHashTime<-totalHashTime+hashTimer.ElapsedMilliseconds
                totalRequests<-totalRequests+1
                totalHashes<-totalHashes+1
                hashTimer.Stop()
                CCRef<!ClientCoordinatorMessage("HashResponse",0)   
    ...
    let rec loop () = actor {
        ...
        if message.Command="InitializeClientCoorinator" then
            ...
            if totalRequests>=20000 then
                ...
                //mailbox.Self <! ClientCoordinatorMessage("Terminate",noOfClients)
            else
                let choices = [1;2;3]
            ...
            delay 10
            printfn ""
            printfn "Total tweets is %d" totalTweets
            printfn "Total tweet time is %d" totalTweetTime
            printfn "Total mentions time is %d" totalMentionsTime
            printfn "Total subscribe time is %d" totalSubscribeTime
            printfn "Total hash time is %d" totalHashTime
            printfn "Simulation Completed. Press Any key to close"
            system.Terminate() |> ignore
        else
            printfn "Crap"
